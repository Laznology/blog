...
<template>
  <div v-if="page" class="relative">
    <header class="py-20 sm:py-32 border-b border-border/40 relative bg-background/50 backdrop-blur-sm">
      <div class="container mx-auto px-4 lg:px-16 max-w-4xl text-center">
        <div class="flex justify-center gap-3 flex-wrap mb-10">
          <Badge 
            v-for="tag in page.tags" :key="tag" variant="outline"
        ...
        <div
          ...
          <div class="flex items-center gap-6">
            <Avatar class="h-16 w-16 border border-border/50 rounded-full shadow-sm">
              <AvatarImage :src="githubAvatarUrl" :alt="displayAuthor" class="object-cover" />
              <AvatarFallback class="bg-muted text-lg font-serif font-light text-foreground/70">
                {{ displayAuthor.substring(0, 2).toUpperCase() }}
              </AvatarFallback>
            </Avatar>
            <div class="text-left">
              <div class="text-sm font-sans tracking-widest uppercase text-foreground/80 mb-1">
                {{ displayAuthor }}
              </div>
    ...
    <div class="container mx-auto px-4 lg:px-16 max-w-6xl py-16 sm:py-24">
      <div class="grid grid-cols-1 xl:grid-cols-5 gap-16 xl:gap-20 relative items-start">
        <nav v-if="hasToc" class="hidden xl:block xl:col-span-1 sticky top-32">
          ...
          <ul class="space-y-4 font-sans font-light text-sm">
            <li v-for="link in tocLinks" :key="link.id">
              <a :href="`#${link.id}`" class="text-foreground/60 hover:text-primary transition-colors block">
                {{ link.text.replace(/^[\d.]+\s*/, '') }}
              </a>
              <ul v-if="link.children" class="mt-3 ml-4 space-y-3 border-l border-border/30 pl-4">
