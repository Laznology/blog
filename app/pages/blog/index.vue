...
<template>
  <div class="container mx-auto px-4 lg:px-16 py-16 sm:py-24 relative">
    <div class="border-b border-border/40 pb-12 mb-16 text-center max-w-3xl mx-auto">
      <h1 class="text-5xl sm:text-7xl font-serif font-light tracking-wide text-foreground/90">
        The Archive.
      </h1>
      <p class="text-lg sm:text-xl font-sans mt-6 text-foreground/60 leading-relaxed font-light">
    ...
    <div class="mb-24 flex flex-col items-center">
      <button @click="showFilters = !showFilters"
        class="text-xs font-sans uppercase tracking-[0.2em] transition-colors flex items-center gap-6 group" :class="showFilters || activeTags.length > 0
          ? 'text-primary'
          : 'text-foreground/50 hover:text-foreground'
          ">
        <span class="w-8 h-px bg-foreground/30 transition-all duration-500 ease-out" :class="{
      ...
      <div class="overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] w-full max-w-3xl pt-0"
        :class="showFilters
          ? 'max-h-200 opacity-100 mt-12'
        ...
        <div class="flex flex-wrap justify-center gap-x-12 gap-y-6 pb-4">
          <button v-for="tag in allTags" :key="tag" @click="toggleTag(tag)"
            ...
            <span class="flex items-center justify-center w-4 h-4 border transition-colors rounded-none" :class="activeTags.includes(tag)
              ? 'border-primary bg-primary'
              : 'border-border/60 group-hover:border-foreground/40'
              ">
              <svg v-if="activeTags.includes(tag)" class="w-3 h-3 text-background" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="square" stroke-linejoin="miter" stroke-width="2" d="M5 13l4 4L19 7"></path>
              </svg>
            </span>
            <span class="text-sm md:text-base font-serif italic tracking-wide transition-colors" :class="activeTags.includes(tag)
              ? 'text-foreground/90'
    ...
    <div class="max-w-4xl mx-auto space-y-16">
      <NuxtLink v-for="article in filteredArticles" :key="article.path" :to="article.path" class="group block">
        <div
          ...
          <div class="flex-1 w-full space-y-6 pt-2">
            <div class="flex justify-between items-center border-b border-border/30 pb-4">
              <span class="text-foreground/50 font-sans tracking-[0.2em] text-[10px] uppercase">{{ article.date
              }}</span>
              <div class="flex gap-2">
                <Badge v-for="tag in article.tags?.slice(0, 1)" :key="tag" variant="outline"
            ...
            <div class="pt-6">
              <span
                ...
                Read entry
                <span
                  class="w-8 h-px bg-foreground/20 group-hover:w-16 group-hover:bg-primary transition-all duration-700"></span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div v-if="filteredArticles.length === 0"
        ...
        <h2 class="text-xl font-serif tracking-widest uppercase text-foreground/40 font-light">
          No recordings found
        </h2>
      </div>
    </div>
